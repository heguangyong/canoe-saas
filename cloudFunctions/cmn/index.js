...
exports.main = async (event, context) => {
	...
	//更新产品状态 
	app.router('updateProductState', async (ctx) => {
        ctx.body = {router:'updateProductState', code: 0, message: 'success' };
		// console.log('updateProductState',event.id,event.on_sale,event.state);
		dbCmnProduct.doc(event.id).update({
		    data: {
				on_sale:event.on_sale,
				state:event.state
			},
		}).then((res)=>{
		    console.log('updateProductState success',res)
		})
    });
	
	//更新产品标签
	app.router('updateProductTags', async (ctx) => {
	    ctx.body = {router:'updateProductTags', code: 0, message: 'success' };
		// console.log('updateProductTags',event.id,event.tags);
		dbCmnProduct.doc(event.id).update({
		    data: {
				tags:event.tags,
			},
		}).then((res)=>{
		    console.log('updateProductTags success',res)
		})
	});
