    ...
    export default {
        ...
        methods: {
            ...
            async getMyCard() {
                ...
                that.myCard = JSON.parse(res.result.data);
                // #endif
            },
            async getMyHolder() {
                let that = this;
                ...
                cardHolder = JSON.parse(res.result.data);
                // #endif
                //按字母分组的名片
                that.listGroup = cardHolder.cards;
                that.holderIdList = cardHolder.search_data;
                let chatIdList = [that.myCard._id];
            ...
            async removeCard(){
                ...
                if(that.index>0 && that.selectedSubIdx>0){
                    ...
                    that.holderIdList.splice(that.existIdx,1);
            ...
            async startTalk(targetCardId){
                // #ifdef MP-WEIXIN
                uni.navigateTo({
                    "url":"/pagesSys/site/service?sourceCardId="+this.user.userInfo.cardId+"&targetCardId=" + targetCardId
                })
                // #endif
                // #ifdef H5
                uni.navigateTo({
                    "url":"/pagesSys/site/WebService?sourceCardId="+this.user.userInfo.cardId+"&targetCardId=" + targetCardId
                })
                // #endif
            },
        }
