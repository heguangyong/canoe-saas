<template>
	<view class="" :class="isUps == true ? 'prevent' : ''">
		<cu-custom    :isBack="true">
			...
			<block slot="content">退货地址</block>
		</cu-custom>
		<view class="page">
			<view class="cu-list menu-avatar no-padding">
				<block v-for="(recipient,index) in bizCard.recipients" :key="index">
					<view @tap="setAsDefault(recipient)" class="cu-item  card-radius margin-top-xxs margin-left-xs margin-right-xs margin-bottom-xs shadow bg-white.light">
						...
						<view class="action">
							<text v-if="recipient.as_default" class="cuIcon-check text-green" style="font-size: 38rpx;"></text>
						</view>
					</view>
				</block>
			</view>
    ...
    export default {
        ...
        data() {
            return {
                ...
                bizCard:{recipients:[{key:'',name:'',phone:'',address:''}]}
            }
        },
		onShow(){
            this.getCardDetail();
        ...
        methods:{
            ...
            async setAsDefault(recipient){
                let that = this;
				that.bizCard.recipients.forEach(each=>{
			...
			async saveAddress(){
                let that = this;
                that.bizCard.recipients.push(that.recipient);
                await wx.cloud.callFunction({
                    name:'sys',
                    data:{
                        $url:'updateCard',
