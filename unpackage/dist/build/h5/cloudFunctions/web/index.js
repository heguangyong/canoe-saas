...
exports.main = async (event, context) => {
    ...
    app.router('signWeixin', async (ctx) => {
        ...
        let card = await dbSysCard.doc(cardId).get();
        let signRet = {
            jsapi_ticket: '',
        ...
        //没有ticket 或者需要刷新
        if(!card.jsapi_ticket || event.needRefreshSignature){
            //没有access_token 或者需要刷新
            if(!card.access_token || event.needRefreshSignature){
                //getAccessToken
                let token_options = {
                    method:'GET',
                ...
                card.access_token=JSON.parse(token_res).access_token;
                console.log('token_res',token_res,'card.access_token',card.access_token);
            ...
            if(card.access_token){
                ...
                let ticket_options = {
                    method:'GET',
                    url: 'https://api.weixin.qq.com/cgi-bin/ticket/getticket?access_token='+card.access_token+'&type=jsapi',
                };
                let ticket_res = await rp(ticket_options);

                //获取成功，缓存到card中
                card.jsapi_ticket=JSON.parse(ticket_res).ticket;
                console.log('ticket_res',ticket_res,'card.jsapi_ticket',card.jsapi_ticket);
                if(card.jsapi_ticket){
                    //缓存token至card中 jsapi_ticket
                    dbSysCard.doc(cardId).update({
                        data:{
                            jsapi_ticket:card.jsapi_ticket,
                        }
                    });

                    //制作签名
                    signRet.jsapi_ticket = card.jsapi_ticket;
                    let keys = Object.keys(signRet);
                    keys = keys.sort();
                    let newArgs = {};
                    keys.forEach(function (key) {
                        newArgs[key.toLowerCase()] = signRet[key];
                    });
